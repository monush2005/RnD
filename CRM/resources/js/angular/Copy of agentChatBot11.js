...
function onMessage(msg) 
{
	...
	if(compose.length>0)
	{
	addTypingStatus(from);
	}
	else
	if(pause.length>0)	
	{
	removeTypingStatus(from);
	}
	else
	...
	//localStorage.setItem("uniqueID",uniqueid);
			...
			if(msg1.trim()!="userdisconnected"){
			 	if ($.inArray(uniqueid, usrList) <= -1) { 
			 		console.log(""+uniqueid);
					usrList.push(uniqueid);
					var d = new Date();
...
/*		var cnfrm = confirm("Are you sure to end this chat Now");
		if (cnfrm) {*/
			var chatid = $(this).closest(".consumer-chat-container").attr("data-chatuniqueid");
