crm.controller('crmCollectiveRptController',function($scope,$location,$http,$rootScope,Excel,$timeout,crmCollectiveRptFactory,viewCceDetailsFactory ,$mdDialog) {
	...
	$rootScope.flagHideDrawer=true;
		    ...
		    $scope.exportToExcel=function(tableId){ // ex: '#my-table'
		          var exportHref=Excel.tableToExcel(tableId,'CRM collective report');
		          $timeout(function(){location.href=exportHref;},100); // trigger download
		   }
		    
	$scope.crmColRptRequestPojo={};
    $scope.crmColRptResponsePojo={};
    $scope.colRptReq=function(obj){
    	showLoader();
    			console.log("===============================");
    			console.log(obj);
    	    	...
    	    	$scope.crmColRptRequestPojo.lang="en";
    			if(obj.agentId=="all"){
        			$scope.crmColRptRequestPojo.cceid="";
        			}
        			else{
        	    	$scope.crmColRptRequestPojo.cceid=obj.agentId;
        			}
    			$scope.crmColRptRequestPojo.sdate=$scope.finalStrt;
    	    	$scope.crmColRptRequestPojo.edate=$scope.finalend;

    	    	console.log($scope.crmColRptRequestPojo);
    	    	crmCollectiveRptFactory.crmColRpt($scope.crmColRptRequestPojo).success(function(data){
    				hideLoader();
    				 $scope.crmColRptResponsePojo=data;
    				 $scope.ivrReportData=$scope.crmColRptResponsePojo.pd;
    				 $scope.Paging=$scope.crmColRptResponsePojo.pd;
    				 console.log($scope.crmColRptResponsePojo);
    				if($scope.crmColRptResponsePojo.rs=="S"){
    					if($scope.crmColRptResponsePojo.pd.length<1){
    					...
    					$scope.commomErrorPopUp($scope.crmColRptResponsePojo.rd);
    				}
