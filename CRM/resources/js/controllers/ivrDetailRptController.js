crm.controller('ivrDetailRptController',function($scope,$location,$http,$rootScope,Excel,$timeout,ivrKpiRptFactory,viewCceDetailsFactory ,$mdDialog) {
	...
	$rootScope.flagHideDrawer=true;
		    ...
		    $scope.exportToExcel=function(tableId){ // ex: '#my-table'
		          var exportHref=Excel.tableToExcel(tableId,'IVR details report');
		          $timeout(function(){location.href=exportHref;},100); // trigger download
		   }
		    
	$scope.ivrDtlRptRequestPojo={};
    $scope.ivrDtlRptResponsePojo={};
    $scope.ivrDtlRpt=function(obj){
    	showLoader();
    			console.log("===============================");
    			console.log(obj);
    	    	...
    	    	$scope.ivrDtlRptRequestPojo.lang="en";
    			if(obj.agentId=="all"){
        			$scope.ivrDtlRptRequestPojo.cceid="";
        			}
        			else{
        	    	$scope.ivrDtlRptRequestPojo.cceid=obj.agentId;
        			}
    			$scope.ivrDtlRptRequestPojo.sdate=$scope.finalStrt;
    	    	$scope.ivrDtlRptRequestPojo.edate=$scope.finalend;

    	    	console.log($scope.ivrDtlRptRequestPojo);
    			ivrKpiRptFactory.ivrDtlRpt($scope.ivrDtlRptRequestPojo).success(function(data){
    				hideLoader();
    				 $scope.ivrDtlRptResponsePojo=data;
    				 $scope.ivrReportData=$scope.ivrDtlRptResponsePojo.pd;
    				 $scope.Paging=$scope.ivrReportData;
    				 console.log($scope.ivrDtlRptResponsePojo);
    				if($scope.ivrDtlRptResponsePojo.rs=="S"){
    					if($scope.ivrDtlRptResponsePojo.pd.length<1){
    						$scope.commomErrorPopUp("No data found");	
    					}
    				}
    				else{
    					$scope.commomErrorPopUp($scope.ivrDtlRptResponsePojo.rd);
    				}
