crm.controller('ivrKpiRptController',function($scope,$location,$http,$rootScope,Excel,$timeout,ivrKpiRptFactory,viewCceDetailsFactory ,$mdDialog) {
	...
	$scope.ivrKpiRptRequestPojo={};
    $scope.ivrKpiRptResponsePojo={};
    $scope.ivrKpiRpt=function(obj){
    	showLoader();
    			console.log("===============================");
    			console.log(obj);
    	    	...
    	    	$scope.ivrKpiRptRequestPojo.lang="en";
    			$scope.ivrKpiRptRequestPojo.trkr="798";/*obj.agentId*/
    			if(obj.agentId=="all"){
    			$scope.ivrKpiRptRequestPojo.cceid="";
    			$scope.ivrKpiRptRequestPojo.type="all";
    			}
    			else{
    	    	$scope.ivrKpiRptRequestPojo.cceid=obj.agentId;
    	    	$scope.ivrKpiRptRequestPojo.type=""
    			}
    			$scope.ivrKpiRptRequestPojo.sdate=$scope.finalStrt;
    	    	$scope.ivrKpiRptRequestPojo.edate=$scope.finalStrt;
    	    	console.log($scope.ivrKpiRptRequestPojo);
    			ivrKpiRptFactory.ivrKpiRpt($scope.ivrKpiRptRequestPojo).success(function(data){
    				hideLoader();
    				 $scope.ivrKpiRptResponsePojo=data;
    				 $scope.ivrReportData=$scope.ivrKpiRptResponsePojo.pd;
    				 console.log($scope.ivrKpiRptResponsePojo);
    				if($scope.ivrKpiRptResponsePojo.rs=="S"){
    					if($scope.ivrKpiRptResponsePojo.pd.length<1){
    					...
    					$scope.commomErrorPopUp($scope.ivrKpiRptResponsePojo.rd);
    				}
