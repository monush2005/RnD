...
import in.spicedigital.umang.dto.request.FetchChildUserReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class FetchChildUserController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_CHILD_USER, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchChildUserReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_CHILD_USER);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_CHILD_USER, entity,String.class);
