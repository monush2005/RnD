...
@Controller
public class FetchDepartmentKeywords {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_DEPTT_SERKEY, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchDeptServiceKeywordsReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_DEPTT_SERKEY);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_DEPTT_SERKEY, entity,String.class);
