...
import in.spicedigital.umang.dto.request.FetchLinkedCategoriesReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class FetchLinkedCategoriesController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_LINKED_APP_CATEGORIES, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchLinkedCategoriesReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_LINKED_APP_CATEGORIES);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_LINKED_APP_CATEGORIES, entity,String.class);
