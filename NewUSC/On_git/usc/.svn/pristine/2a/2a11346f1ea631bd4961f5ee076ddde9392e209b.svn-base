...
import in.spicedigital.umang.dto.request.FetchDeptServicesReqPojo;
...
public class FetchServicesController {
	...
	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_SERVICES, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchDeptServicesReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_SERVICES);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_SERVICES, entity,String.class);
