...
import in.spicedigital.umang.dto.request.ViewRequestToNegdReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class FetchUserRequestController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_USER_REQUEST, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody ViewRequestToNegdReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_USER_REQUEST);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_USER_REQUEST, entity,String.class);
