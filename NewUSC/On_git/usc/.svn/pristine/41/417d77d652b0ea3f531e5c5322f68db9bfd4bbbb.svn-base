...
import in.spicedigital.umang.dto.request.FetchChildUserTreeReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class FetchChildUserTreeController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_CHILD_USER_TREE, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchChildUserTreeReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_CHILD_USER_TREE);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_CHILD_USER_TREE, entity,String.class);
