...
import in.spicedigital.umang.dto.request.MinistryReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class ManageMinistryController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_MANAGE_MINISTRY, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody MinistryReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_MANAGE_MINISTRY);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_MANAGE_MINISTRY, entity,String.class);
