...
import in.spicedigital.umang.dto.request.SearchUserIdReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class FetchUserIdsController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_USERID, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody SearchUserIdReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_USERID);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_USERID, entity,String.class);
