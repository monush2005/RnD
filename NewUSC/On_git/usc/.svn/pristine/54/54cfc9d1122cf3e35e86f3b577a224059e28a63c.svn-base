...
@Controller
public class RequestBannerApprovalController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_BANNER_REQUEST_APPROVAL, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody RequestBannerApprovalReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_BANNER_REQUEST_APPROVAL);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_BANNER_REQUEST_APPROVAL, entity,String.class);
