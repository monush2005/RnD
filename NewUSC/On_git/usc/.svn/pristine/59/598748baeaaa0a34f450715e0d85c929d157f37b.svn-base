...
import in.spicedigital.umang.dto.request.FetchUserDetailsReqPojo;
import in.spicedigital.umang.dto.request.UpdateUserAccountReqPojo;
import in.spicedigital.umang.utility.Utility;

@Controller
public class UpdateUserAccountController {
	
	/*UpdateUserAccountReqPojo*/
	...
	@RequestMapping(value = RestURIConstants.REST_POST_UPDATE_USER_ACCOUNT, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody UpdateUserAccountReqPojo requestBody,HttpSession session,HttpServletRequest request) {
			ResponseEntity<String> response =null;
			logger.info("UpdateUserAccountReqPojo Request: {}",requestBody);
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.UPDATE_USER_ACCOUNT);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.UPDATE_USER_ACCOUNT, entity,String.class);
				responseString=response.getBody();
			} catch (Exception e) {
				logger.error("UpdateUserAccountReqPojo Response",e);
				responseString=PropertyConfiguration.getDefaultErrorResponse("Sorry, Something went wrong");
				e.printStackTrace();
			}
			logger.info("UpdateUserAccountReqPojo Response:  {}",response);
