...
public class ManagerBannerController {
	...
	@RequestMapping(value = RestURIConstants.REST_POST_BANNER_MANAGE, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody ManagerBannerReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_BANNER_MANAGE);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_BANNER_MANAGE, entity,String.class);
