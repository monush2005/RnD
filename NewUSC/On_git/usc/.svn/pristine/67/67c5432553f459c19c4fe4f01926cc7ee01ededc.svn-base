...
import in.spicedigital.umang.dto.request.LinkUnlinkReqPojo;
...
public class LinkUnlinkController {
	...
	@RequestMapping(value = RestURIConstants.REST_POST_LINK_UNLINK, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody LinkUnlinkReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				System.out.println("requestJsonrequestJson::::"+requestJson);
				headers.setContentType(MediaType.APPLICATION_JSON);
				headers.set("Authorization", PropertyConfiguration.getConfiguration(WsConstants.WS_AUTHORIZATION));
				HttpEntity<String> entity = new HttpEntity<String>(requestJson,headers);
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_LINK_UNLINK);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_LINK_UNLINK, entity,String.class);
