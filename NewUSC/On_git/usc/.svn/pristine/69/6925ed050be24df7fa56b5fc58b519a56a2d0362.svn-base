...
import in.spicedigital.umang.dto.request.RemoveBannerReqPojo;
import in.spicedigital.umang.utility.Utility;

@Controller
public class BannerRemoveController {
	...
	@RequestMapping(value = RestURIConstants.REST_POST_REMOVE_BANNER, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody RemoveBannerReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_REMOVE_BANNER);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_REMOVE_BANNER, entity,String.class);
