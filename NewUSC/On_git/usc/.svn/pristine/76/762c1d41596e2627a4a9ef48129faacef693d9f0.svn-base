...
import in.spicedigital.umang.usc.request.AccountEditOtpReqPojo;
...
public class InitiateOtpController {
	
	@Autowired
	private Utility encryptionUtility;
	
	@RequestMapping(value = "/initOtp", method = RequestMethod.POST)
	public @ResponseBody String otpInitiate(@RequestBody AccountEditOtpReqPojo requestBody,HttpSession session,HttpServletRequest request) {
		CommonApiUtility restTemplate=new CommonApiUtility();
		try {
			Gson gson=new GsonBuilder().serializeNulls().create();
			String requestJson=gson.toJson(requestBody);
			return restTemplate.postForEntity(RestServiceURIConstants.REST_POST_USC_OTP_INIT,requestJson,request,encryptionUtility.getReadTimeout(),encryptionUtility.getConnectionTimeout());
		} catch (Exception e) {
			e.printStackTrace();
