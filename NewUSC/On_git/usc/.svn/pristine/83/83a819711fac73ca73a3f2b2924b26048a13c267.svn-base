...
import in.spicedigital.umang.dto.request.ModerateBannerReqPojo;
import in.spicedigital.umang.utility.Utility;

@Controller
public class ModerateBannerReqController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_BANNER_MODERATE, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody ModerateBannerReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_BANNER_MODERATE);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_BANNER_MODERATE, entity,String.class);
