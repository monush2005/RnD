...
import in.spicedigital.umang.dto.request.SearchServicesReqPojo;
...
public class SearchServicesController {
	...
	@RequestMapping(value = RestURIConstants.REST_POST_SEARCH_SERVICES, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody SearchServicesReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_SEARCH_SERVICES);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_SEARCH_SERVICES, entity,String.class);
