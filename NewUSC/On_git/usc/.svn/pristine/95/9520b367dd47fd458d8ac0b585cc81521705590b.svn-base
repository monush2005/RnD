...
import in.spicedigital.umang.dto.request.FetchCategoryReqPojo;
...
public class FetchCategoryController {
	...
	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_CATEGORY, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchCategoryReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_CATEGORY);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_CATEGORY, entity,String.class);
