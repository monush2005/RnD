...
import in.spicedigital.umang.dto.request.ValOtpReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class RegistrationValOtpController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_REG_VAL, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody ValOtpReqPojo requestBody,HttpSession session) {
			ResponseEntity<String> response =null;
			logger.info("Validate OTP Request: {}",requestBody);
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_REG_VAL);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_REG_VAL, entity,String.class);
				responseString=response.getBody();
			} catch (Exception e) {
				logger.error("Validate OTP Response",e);
				responseString=PropertyConfiguration.getDefaultErrorResponse("Sorry, Something went wrong");
				e.printStackTrace();
			}
			logger.info("Validate OTP  Response:  {}",response);
