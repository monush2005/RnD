...
import in.spicedigital.umang.dto.request.FetchCceMnoReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class ForgotInitController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_FORGOT_INIT, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchCceMnoReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FORGOT_INIT);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FORGOT_INIT, entity,String.class);
