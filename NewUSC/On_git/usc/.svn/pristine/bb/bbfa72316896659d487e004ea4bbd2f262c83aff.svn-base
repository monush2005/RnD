...
import in.spicedigital.umang.dto.request.UpdateSelfcareBrandingReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class UpdateSelfcareBrandingController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_UPDATE_SELFCARE_BRANDING, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody UpdateSelfcareBrandingReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_UPDATE_SELFCARE_BRANDING);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_UPDATE_SELFCARE_BRANDING, entity,String.class);
