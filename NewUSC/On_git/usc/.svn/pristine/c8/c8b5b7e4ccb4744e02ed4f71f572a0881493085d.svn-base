...
import in.spicedigital.umang.dto.request.FetchBannerCommentsReqPojo;
import in.spicedigital.umang.utility.Utility;

@Controller
public class BannerCommentsController {
	...
	@RequestMapping(value = RestURIConstants.REST_POST_FETCHCOMMENTS_BANNER, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchBannerCommentsReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCHCOMMENTS_BANNER);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCHCOMMENTS_BANNER, entity,String.class);
