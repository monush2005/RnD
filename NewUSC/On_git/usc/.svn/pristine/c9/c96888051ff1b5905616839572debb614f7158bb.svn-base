...
import in.spicedigital.umang.dto.request.UpdateDeptSrvKeywordReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class UpdateServiceKeywordsController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_UPDATE_SERVICE_KEYWORD, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody UpdateDeptSrvKeywordReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_UPDATE_SERVICE_KEYWORD);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_UPDATE_SERVICE_KEYWORD, entity,String.class);
