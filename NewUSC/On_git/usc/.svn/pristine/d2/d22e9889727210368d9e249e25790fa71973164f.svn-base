...
@Controller
public class DelDepttCatController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_DEL_DEPTT_CAT, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody DelDeptReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_DEL_DEPTT_CAT);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_DEL_DEPTT_CAT, entity,String.class);
