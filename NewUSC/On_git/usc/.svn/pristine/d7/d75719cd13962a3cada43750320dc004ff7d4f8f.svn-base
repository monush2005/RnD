...
import in.spicedigital.umang.dto.request.FetchFeedbackPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class FetchFeedbackController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_FEEDBACK, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchFeedbackPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_FEEDBACK);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_FEEDBACK, entity,String.class);
