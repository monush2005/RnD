...
import in.spicedigital.umang.dto.request.UpdateDeptFaqReqPojo;
...
public class UpdateFaqController {
	...
	@RequestMapping(value = RestURIConstants.REST_POST_UPDATE_FAQ, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody UpdateDeptFaqReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_UPDATE_FAQ);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_UPDATE_FAQ, entity,String.class);
