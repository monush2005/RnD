...
import in.spicedigital.umang.dto.request.EditServicesReqPojo;
...
public class UpdateServiceController {
	...
	@RequestMapping(value = RestURIConstants.REST_POST_UPDATE_SERVICES, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody EditServicesReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_UPDATE_SERVICES);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_UPDATE_SERVICES, entity,String.class);
