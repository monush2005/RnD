...
public class ActivateDeactivateController {
	...
	@RequestMapping(value = RestURIConstants.REST_POST_ACT_DEACT, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody ActivateDeactivateAppReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_ACT_DEACT);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_ACT_DEACT, entity,String.class);
