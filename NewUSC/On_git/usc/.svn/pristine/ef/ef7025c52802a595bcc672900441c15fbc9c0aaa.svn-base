...
@Controller
public class FetchBannerCommentsController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_BANNER_FETCH_COMMENTS, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchBannerCommentsReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_BANNER_FETCH_COMMENTS);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_BANNER_FETCH_COMMENTS, entity,String.class);
