...
import in.spicedigital.umang.dto.request.AddReportCardReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class AddReportCardController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_ADD_REPORT_CARD, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody AddReportCardReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_ADD_REPORT_CARD);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_ADD_REPORT_CARD, entity,String.class);
