...
import in.spicedigital.umang.dto.request.AddRolesReqPojo;
import in.spicedigital.umang.dto.request.ResponseBannerReqPojo;
import in.spicedigital.umang.utility.Utility;

@Controller
public class ApproveRejectBannerController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_BANNER_APPROVE_REJECT, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody ResponseBannerReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_BANNER_APPROVE_REJECT);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_BANNER_APPROVE_REJECT, entity,String.class);
