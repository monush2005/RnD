...
import in.spicedigital.umang.dto.request.FetchDeptIssuesReqPojo;
import in.spicedigital.umang.dto.request.FetchDeptServiceKeywordsReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class FetchDeptIssuesController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_DEPT_ISSUES, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchDeptIssuesReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_DEPT_ISSUES);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_DEPT_ISSUES, entity,String.class);
