...
import in.spicedigital.umang.dto.request.FetchRightsReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class FetchRightsController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_RIGHTS, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchRightsReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_RIGHTS);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_RIGHTS, entity,String.class);
