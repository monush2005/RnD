...
import in.spicedigital.umang.dto.request.FetchRoleCategoryReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class FetchRoleCategoryWiseController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_ROLE_CATEGORYWISE, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchRoleCategoryReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_ROLE_CATEGORYWISE);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_ROLE_CATEGORYWISE, entity,String.class);
