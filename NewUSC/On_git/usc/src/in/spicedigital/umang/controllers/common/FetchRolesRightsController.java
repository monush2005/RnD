...
import in.spicedigital.umang.dto.request.FetchRoleRightsReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class FetchRolesRightsController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_ROLE_RIGHTS, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchRoleRightsReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_ROLE_RIGHTS);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_ROLE_RIGHTS, entity,String.class);
