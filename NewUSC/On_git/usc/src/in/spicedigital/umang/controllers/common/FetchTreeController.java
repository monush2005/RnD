...
import in.spicedigital.umang.dto.request.FetchAppTreeReqPojo;
import in.spicedigital.umang.dto.request.FetchStatesReqPojo;
import in.spicedigital.umang.utility.Utility;

@Controller
public class FetchTreeController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_APP_TREE, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchAppTreeReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_APPTREE);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_APPTREE, entity,String.class);
