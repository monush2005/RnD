...
public class FetchUserController {
	...
	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_USER, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchUserDetailsReqPojo requestBody,HttpSession session) {
			ResponseEntity<String> response =null;
			requestBody.setToken(RequestContextHolder.currentRequestAttributes().getSessionId());
			logger.info("FetchUserController Request: {}",requestBody);
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_USERDETAILS);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_USERDETAILS, entity,String.class);
