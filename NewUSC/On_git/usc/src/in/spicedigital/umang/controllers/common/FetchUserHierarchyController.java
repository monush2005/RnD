...
import in.spicedigital.umang.dto.request.FetchUserHierarchyReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class FetchUserHierarchyController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_USER_HIERARCHY, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchUserHierarchyReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_USER_HIERARCHY);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_USER_HIERARCHY, entity,String.class);
