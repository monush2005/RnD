...
import in.spicedigital.umang.dto.request.FetchUserRoleReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class FetchUserRolesController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_FETCH_USER_ROLES, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody FetchUserRoleReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_USER_ROLES);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_FETCH_USER_ROLES, entity,String.class);
