...
import in.spicedigital.umang.dto.request.LogFaqReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class LogFaqController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_LOG_FAQ, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody LogFaqReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_LOG_FAQ);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_LOG_FAQ, entity,String.class);
