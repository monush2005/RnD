...
import in.spicedigital.umang.dto.request.AddRoleRightMappingReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class MapRoleRightsController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_MAP_ROLE_RIGHT, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody AddRoleRightMappingReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_MAP_ROLE_RIGHT);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_MAP_ROLE_RIGHT, entity,String.class);
