...
import in.spicedigital.umang.dto.request.UpdateReportCardReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class UpdateReportCardController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_UPDATE_REPORT_CARD, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody UpdateReportCardReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_UPDATE_REPORT_CARD);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_UPDATE_REPORT_CARD, entity,String.class);
