...
import in.spicedigital.umang.dto.request.ViewFilledQuestionnaireReqPojo;
import in.spicedigital.umang.utility.Utility;
@Controller
public class ViewFilledQuestionnaireController {
	
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	@Autowired
	private Utility encryptionUtility;

	@RequestMapping(value = RestURIConstants.REST_POST_QUESTIONNAIRE_VIEW, method = RequestMethod.POST)
		public @ResponseBody String  login(@RequestBody ViewFilledQuestionnaireReqPojo requestBody,HttpSession session) {
			...
			try {
				...
				logger.info("URL: {}",WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_QUESTIONNAIRE_VIEW);
				response = restTemplate.postForEntity(WsConstants.WS_APPLICATION_URL+RestServiceURIConstants.REST_POST_QUESTIONNAIRE_VIEW, entity,String.class);
